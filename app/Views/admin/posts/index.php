      ...
      <tbody>
        <?php if (!empty($posts)): ?>
          <?php foreach ($posts as $post): ?>
            <tr class="post-item">
              <td class="ps-4 fw-semibold text-wrap" style="max-width:250px;"><?= esc($post['title']) ?></td>
              <td><span class="badge bg-light text-dark border"><?= esc($post['category']) ?></span></td>
...
<div class="modal fade" id="postModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content border-0 shadow">
      <form action="<?= base_url('admin/posts/save') ?>" method="post">
        <?= csrf_field() ?>
        ...
        <div class="modal-footer border-top p-2 p-md-3 d-flex flex-column flex-sm-row gap-2">
          ...
          <button type="submit" class="btn-church w-100 w-sm-auto order-1 order-sm-2 text-center" style="background-color:#0d1830;">Simpan Postingan</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  ...
  // Filter pencarian live (Memfilter tabel Desktop & kartu Mobile sekaligus)
